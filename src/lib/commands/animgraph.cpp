...
#include <lib/orxpp/configutils.h>
#include <lib/orxpp/list.h>
...
void orxFASTCALL GetAnimForTagCommand(orxU32 _u32ArgNumber, const orxCOMMAND_VAR *_astArgList, orxCOMMAND_VAR *_pstResult) {

    orxASSERT(_u32ArgNumber == 2 && 
              _astArgList[0].eType == orxCOMMAND_VAR_TYPE_U64 &&
              _astArgList[1].eType == orxCOMMAND_VAR_TYPE_STRING);

    orxSTRUCTURE * structure = orxStructure_Get(_astArgList[0].u64Value);

    orxASSERT(orxStructure_GetID(structure) == orxSTRUCTURE_ID_OBJECT);

    orxObjectX_GetAnimForTag(orxOBJECT(structure), _astArgList[1].zValue);
}


void orxFASTCALL SetAnimByTagCommand(orxU32 _u32ArgNumber, const orxCOMMAND_VAR *_astArgList, orxCOMMAND_VAR *_pstResult) {

    orxASSERT((_u32ArgNumber == 2 || _u32ArgNumber == 3) && 
              _astArgList[0].eType == orxCOMMAND_VAR_TYPE_U64 &&
              _astArgList[1].eType == orxCOMMAND_VAR_TYPE_STRING);
    ...
    if(_u32ArgNumber == 2 || !_astArgList[2].bValue) {
        orxObjectX_SetTargetAnimByTag(orxOBJECT(structure), _astArgList[1].zValue);
    } else {
        orxObjectX_SetCurrentAnimByTag(orxOBJECT(structure), _astArgList[1].zValue);
    }

}


void orxFASTCALL AnimHasTagCommand(orxU32 _u32ArgNumber, const orxCOMMAND_VAR *_astArgList, orxCOMMAND_VAR *_pstResult) {
    ...
    if(_u32ArgNumber == 2 || !_astArgList[2].bValue) {
        _pstResult->bValue = orxObjectX_TargetAnimHasTag(orxOBJECT(structure), _astArgList[1].zValue);
    } else {
        _pstResult->bValue = orxObjectX_CurrentAnimHasTag(orxOBJECT(structure), _astArgList[1].zValue);
    }

    _pstResult->eType = orxCOMMAND_VAR_TYPE_BOOL;
}
...
void RegisterAnimGraphCommands() {
    ...
    {
        orxCOMMAND_VAR_DEF args[3];
        args[0].eType = orxCOMMAND_VAR_TYPE_U64;
        args[0].zName = "Object";
        args[1].eType = orxCOMMAND_VAR_TYPE_STRING;
        args[1].zName = "AnimationTag";
        args[2].eType = orxCOMMAND_VAR_TYPE_BOOL;
        args[2].zName = "[Current=false]";

        orxCOMMAND_VAR_DEF ret_of_animfortag;
        ret_of_animfortag.eType = orxCOMMAND_VAR_TYPE_STRING;
        ret_of_animfortag.zName = "AnimationName";
        ...
        orxCommand_Register("ObjectX.AnimHasTag", AnimHasTagCommand, 2, 1, args, &ret_of_query);
